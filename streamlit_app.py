...
# ── data loading ──────────────────────────────────────────────────────────────
# DISABLE CACHE TEMPORARILY TO FORCE RELOAD
# # @st.cache_resource(show_spinner="Loading match database…")  # DISABLED FOR DEBUG
def load_data():
...
def tab_history(data):
    ...
    if subtab == "Most Successful Nations":
        ...
        # DIRECTLY READ FROM GOAL.CSV TO AVOID CACHING ISSUES
        goal_file = DATA_DIR / "goal.csv"
        if goal_file.exists():
            gs = pd.read_csv(goal_file)
        else:
        ...
        fig, ax = plt.subplots(figsize=(10,7))
        ax.barh(top_s["scorer"], top_s["goals"], color=ACCENT, alpha=0.9, label="Goals", height=0.7)
        if "penalties" in top_s.columns:
        ...
        ax.set_xlabel("Goals"); ax.invert_yaxis(); 
        if "penalties" in top_s.columns:
            ax.legend(fontsize=9)
        ax.grid(True, alpha=0.2, axis="x")
        plt.tight_layout(pad=0.5)
